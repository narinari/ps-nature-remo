module NatureRemo.Generation.Definitions where
...
import NatureRemo.Generation.SchemaType (generateTypeForSchema)
import Data.Swagger.JsonSchema (Schema(..), SchemaType(..), TypeValidator(..))
import NatureRemo.Generation.Names (jsonFieldToPsField, modulePrefix, refName, startsWith, typeModule)
import Partial.Unsafe (unsafeCrashWith)
import Unsafe.Coerce (unsafeCoerce)

type NatureRemoSchema =
  { name :: String
  , schema :: Schema }
...
generateDefinitionModules :: Partial => AST.ApiModuleName -> Array NatureRemoSchema -> Array AST.ApiModule
generateDefinitionModules moduleNs schemas = k8sTypeModules <> [lensModule]
  where
    ...
    dumpModule :: Tuple String (Array NatureRemoSchema) -> String
...
generateModule :: Partial => AST.ApiModuleName -> Array String -> String -> Array NatureRemoSchema -> GeneratedModule
...
byNamePrefix {name: name1} {name: name2} = name1 == name2